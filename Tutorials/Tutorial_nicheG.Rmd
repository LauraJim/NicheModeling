...
Crop the rasters before stacking them with a polygon of the study area. Here they are cropped to the specific study area with the help of a polygon. They can also be cropped with coordinates (see example *Threnetes ruckeri* below).
```{r, results='hide'}
...
# crop the raster of the calculated model
cn.wnc <- mask(crop(cn.wn, cn.shp), cn.shp)

# stack the rasters
area <- stack(cn.wnc, bio1c, bio12c)

```
\

Save the cropped environmental layers for later use (see tutorial for the accumulation curve function `accum.occ3`).
```{r, eval = FALSE}
writeRaster(bio1c,"./Output_Data/Catasticta_nimbice_bio1_cropped.tif", overwrite = T)
writeRaster(bio12c, "./Output_Data/Catasticta_nimbice_bio12_cropped.tif", overwrite = T)
# save the stacked model with environmental layers, also needed for `accum.occ3`
writeRaster(area, "./Output_Data/Catasticta_nimbice_wn_cropped.tif", overwrite = T)
```
\
...
Plot raster that has been cropped by coordinates in ggplot.
```{r, fig.show='hide', results='hide'}
# read occurrence data of species
species <- read.csv("./Input_Data/Threnetes_ruckeri_occ_GE.csv",header=T)

# Option 1: define coordinates that describe the study area
