...
Apply the function `niche.G` with the parameters `Estck`, `mu`, `Sigma`, and `save.map` and plot it.
...
occ2 <- read.csv("./Input_Data/Threnetes_ruckeri_occ_GE.csv",header=T)[,-(1:2)]
...
```{r, eval=FALSE}
result2 <- niche.G(Estck = bios, mu = center2, Sigma = boundary2, save.map = saveM2)
```
\
...
```{r}
occ3 <- read.csv("./Input_Data/Threnetes_ruckeri_occ_GE.csv",header=T)
```
\
