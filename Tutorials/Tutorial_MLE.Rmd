...
M.shp <- readOGR("./Initial_Data/shapefiles","nimbice3")
```
\

Apply the function with `Estck`, `M.shp`, and `N`. Here, `N` will be 10,000. The generated matrix will therefore have two columns with environmental data from the study area and 10,000 rows.
...
```{r, results='hide', message=FALSE}
# Read M polygon
M.shp <- readOGR("./Initial_Data/shapefiles","Threnetes_ruckeri")

```
\

Apply the function with `M.shp`, `Estck`, and a sample size with `N`.
```{r}
...
Sample random points from the area of interest using the `sam.polyM` function with the parameters `M.shp`, `N`, and `Estck`.
```{r}
sam.Mpnts2 <- sam.polyM(M.shp = shapefile, N = 5000, Estck = stck_bios)
```
\
