...
```{r, eval=FALSE}
write.csv(sam.Mpnts,file=paste0("./Output_Data/Catasticta_nimbice","_M_E.csv"),
          row.names = F)
```
\
...
occ <- read.csv("./Input_Data/Catasticta_nimbice_occ_GE.csv",header=T) 
# set colors
...
```{r}
sp.occ <- read.csv("./Input_Data/Catasticta_nimbice_occ_GE.csv",header=T)[,-(1:2)]
```
\
...
```{r}
sp.occ2 <- read.csv("./Input_Data/Threnetes_ruckeri_occ_GE.csv",header=T)[,-(1:2)]

shapefile <- readOGR("./Input_Data/shapefiles","Threnetes_ruckeri")
```
\
