...
The next step is to calculate the Markov Chain Monte Carlocreate for the Bayesian model. For this, two functions are utilized: `Energy` and `Run`. Both functions need to be adjusted based on the dataset and the resulting estimations. The previously defined variables are necessary for these calculations. Another variable that is needed is the number of environmental variables in the study. In this case we have two (bio1 and bio12).
...
Apply the function `Engery` that calculates a negative logarithm (posterior) of the a priori parameters.
```{r}
...
The next function is `Run`, which runs and plots the MCMC simulations from the posterior. 10000 or 20000 iterations are normal.
```{r}
...
With the resulting output of `Run`, the MCMC algorithm can be run. Calling info will draw the iterations in as many plots as dimensions where entered for Run (in this case: 5). Niter are the amount of iterations. 1000 iterations would be quite fast but it is better to use at least 10000.
```{r}
...
proc.time() - ptm
```
The resulting values in the beginning are quite large and not of interest. Smaller iterations are of interest.
