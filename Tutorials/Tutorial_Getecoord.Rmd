...
For this species we use presence data from the GBIF datasets and the environmental conditions "mean annual temperature" (= bio1) and "total annual precipitation" (= bio12) from the WorldClim datasets.  
\

To create the parameter `estck`, two or more rasters are combined into one rasterstack. This rasterstack will have as many layers as rasters are combined together. In this example, there are only two layers (bio1 and bio12).  
```{r}
...
The datasets for the species *Threnetes ruckeri* was gathered from GBIF and WorldClim (same dataset as previous example).
