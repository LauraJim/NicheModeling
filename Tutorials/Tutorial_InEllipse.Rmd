...
cloud <- read.csv("./Input_Data/Threnetes_ruckeri_M_GE.csv",header=T)[,-(1:2)]
```
```{r, echo=FALSE}
head(cloud, n=5)
```
...
```{r}
# read table, omit coordinates
occ <- read.csv("./Input_Data/Threnetes_ruckeri_occ_GE.csv",header=T)[,-(1:2)]
...
```{r, fig.show='hide'}
occ2 <- read.csv ("./Input_Data/Catasticta_nimbice_occ_GE.csv",header=T)[,-(1:2)]
mu2 <- colMeans(occ2)
Sigma2 <- cov(occ2)
cloud2 <- read.csv("./Input_Data/Catasticta_nimbice_M_GE.csv",header=T)[,-(1:2)]
